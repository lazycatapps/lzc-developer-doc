...
应用文稿和应用数据是完全独立的两个特性，不要把应用文稿当成应用自己的持久化数据目录使用。

| 类型 | 目录 | 生命周期归属 | 适合保存 |
| -- | -- | -- | -- |
| 应用数据 | `/lzcapp/var` | 应用 | 数据库、索引、配置、运行状态等应用内部数据 |
| 应用文稿 | `/lzcapp/documents/<uid>` | 用户 | 用户能直接理解、打开、迁移或管理的文件 |

最简单的判断准则是：这个目录里是否会出现用户无法理解的文件？

1. 如果会出现用户无法理解的文件，应保存到 `/lzcapp/var`，不要保存到应用文稿目录。例如应用内数据库、全文索引、缩略图缓存、内部配置、任务状态文件等。
2. 如果文件是用户主动创建、导入、下载或希望长期保留的内容，可以保存到应用文稿目录。例如下载类应用保存的视频、图片、文档、压缩包等资源。
3. 应用文稿目录默认不随应用卸载清理，不能依赖卸载应用来清除其中的内容。
...
3. `/lzcapp/documents`
   ...
   - 只用于保存用户能直接理解和管理的文件，不用于保存应用内部数据库、缓存、索引、配置或运行状态。
