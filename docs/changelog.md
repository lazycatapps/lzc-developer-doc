...
- 修正 `routes` 中 `exec://$port,$exec_file_path` 的自动健康监测处理。旧版本不会将此类路由纳入自动健康监测，新版本会将其对应的本地端口正确纳入监测；依赖旧行为且未在 `$port` 提供服务的 LPK 可能受到影响，该修正会破坏兼容性
...
- 新增[resource export](./resource-skill-mcp.md)，支持通过LPK载体对任意类型资源进行导出->聚合->导入
- 新增[应用间访问机制](./advanced-app-interconnect.md)
