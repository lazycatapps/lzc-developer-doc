...
4. **开发者模式**： 上面是懒猫微服的三个设计层， 但是对于开发者来说， 初期有很多实验性的工作， 为了减少对开发者的限制。 我们的系统也支持 [KVM](https://developer.lazycat.cloud/kvm.html)、 [LightOS](./advanced-lightos.md)。 KVM 主要是安装一个虚拟机后， 可以 SSH 进去任意部署开发环境；LightOS 适合长期维护完整 Linux 运行环境，[Docker](./dockerd-support.md) 也应在 LightOS 中使用。 当然也可以直接用 [PVE](https://appstore.lazycat.cloud/#/shop/detail/in.zhaoj.webvirtcloud)来专业管理和操作 QEMU/KVM 虚拟机。
...
- **传统模式**：基于 KVM 或 LightOS，按传统 NAS 方式维护运行环境；如果使用 Docker，应在 [LightOS](./dockerd-support.md) 中使用。对开发者折腾更自由，但不适合作为面向普通微服用户的标准分发方式。
