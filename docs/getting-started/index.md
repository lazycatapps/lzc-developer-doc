# 新版入门路线 {#getting-started-overview}
...
2. 理解 `project` 命令默认优先使用 `lzc-build.dev.yml`，并通过输出里的 `Build config` 确认当前目标。
3. 理解前端开发如何通过 `inject request` 转到开发机，后端开发如何通过 `project sync --watch` 落到真实运行环境。
4. 在客户端和浏览器两条路径都能验证应用可用。
5. 在需要后端时，正确配置 HTTP 路由。
6. 在需要镜像定制时，使用 embed image 机制而不是手工推送镜像。
7. 使用 `lzc-cli project release` 产出发布用 `.lpk`。
