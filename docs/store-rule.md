...
#### 5. 不发红包的应用类型
当然啦，有些类型的应用暂时不在奖励范围内，比如纯网页游戏、纯书籍页面、纯教程网站、网页离线应用，图床类导航类，书签类，笔记类，在线看视频（moontv魔改的），清单类，短链生成，阅后即焚，mytube类（拉取YOUTUBE内容的），VPN 类应用，记账类，游戏服务器的不同模组或者纯数据库软件。
...
2.  如果应用有上传/下载功能，需要接入懒猫网盘的自动拦截文件选择器，未接入无法在懒猫商店上交。[参考文档](https://gitee.com/lazycatcloud/lzc-developer-doc/edit/master/docs/store-rule.md)
:::
